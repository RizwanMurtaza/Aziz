...
namespace Nop.Plugin.Widgets.GamingHeroSlider.Controllers;

[AuthorizeAdmin]
[Area(AreaNames.ADMIN)]
[AutoValidateAntiforgeryToken]
public class GamingHeroSliderController : BasePluginController
{
    #region Fields
    ...
    public GamingHeroSliderController(ILocalizationService localizationService,
    ...
    {
        var key = $"{nameof(GamingHeroSliderSettings)}.{nameof(GamingHeroSliderSettings.Slides)}";
    ...
    public async Task<IActionResult> Configure()
    {
        ...
        var sliderSettings = await _settingService.LoadSettingAsync<GamingHeroSliderSettings>(storeScope);
        ...
        return View("~/Plugins/Widgets.GamingHeroSlider/Views/Configure.cshtml", model);
    }
    ...
    public virtual async Task<IActionResult> SlideAdd(GamingSlidePictureModel model)
    {
    ...
    [HttpPost]
    [CheckPermission(StandardPermission.Configuration.MANAGE_WIDGETS)]
    public async Task<IActionResult> SlideList(GamingSlidesSearchModel slidesSearchModel)
    {
        var storeScope = await _storeContext.GetActiveStoreScopeConfigurationAsync();
        var slides = await GetSlidesForStoreAsync(storeScope);

        if (slides is null)
            return Json(new GamingSlideListModel());

        var model = await new GamingSlideListModel().PrepareToGridAsync(slidesSearchModel, slides.ToPagedList(slidesSearchModel), () =>
        {
            return slides
                ...
                .SelectAwait(async item =>
                {
                    ...
                    return new GamingSlidePublicModel
                    {
    ...
    [HttpPost]
    [CheckPermission(StandardPermission.Configuration.MANAGE_WIDGETS)]
    public virtual async Task<IActionResult> SlideEdit(GamingSlidePictureModel model)
    {
        //load settings for a chosen store scope
        var storeScope = await _storeContext.GetActiveStoreScopeConfigurationAsync();
